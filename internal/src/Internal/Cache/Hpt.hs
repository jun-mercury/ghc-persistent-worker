...
loadHomeUnit log dflags0 features unit sw@(state0, hsc_env0) path
  ...
  = validateStoredUnit log features hsc_env0 state0.make unit path >>= \case
      ...
      Stale _ -> do
        let state0' = updateMakeState (Make.evictUnit unit) state0
  ...
  where
    restore (s0, h0) = do
      ...
      logTimed log "Loading cached home unit" $ fmap swap do
        runStateT (loadCachedHomeUnit log features.fixedNodesCache hsc_env1 unit path (cachedUnit, dflags)) state1
